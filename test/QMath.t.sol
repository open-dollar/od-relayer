...
contract QMath is Test {
    ...
    IAlgebraFactory public algebraFactory =
        IAlgebraFactory(SEPOLIA_ALGEBRA_FACTORY);
    ...
    function setUp() public {
        ...
        pool = algebraFactory.poolByPair(
            SEPOLIA_SYSTEM_COIN,
            address(mockWeth)
        );
        ...
        if (address(token0) == SEPOLIA_SYSTEM_COIN) {
            require(
                keccak256(abi.encodePacked("OD")) ==
                    keccak256(abi.encodePacked(token0.symbol())),
                "!OD"
            );
        ...
        camelotOdWethOracle = camelotRelayerFactory.deployAlgebraRelayer(
            SEPOLIA_ALGEBRA_FACTORY,
            SEPOLIA_SYSTEM_COIN,
            address(mockWeth),
            uint32(ORACLE_INTERVAL_TEST)
        );

        chainlinkEthUSDPriceFeed = chainlinkRelayerFactory
            .deployChainlinkRelayer(
                SEPOLIA_CHAINLINK_ETH_USD_FEED,
                ORACLE_INTERVAL_TEST
            );

        systemOracle = denominatedOracleFactory.deployDenominatedOracle(
            camelotOdWethOracle,
    ...
    function testPoolPrice() public {
        ...
        uint256 _price = (
            SafeMath.div(uint256(_sqrtPriceX96 * 1e9), (2 ** 96))
        ) ** 2;
        ...
        emit log_named_uint(
            "Price Calculated",
            (INIT_WETH_AMOUNT * WAD) / INIT_OD_AMOUNT
        );
    }
